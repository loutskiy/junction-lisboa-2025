...
def extract_enhanced_features(text):
    ...
    features = {
        ...
        # Дополнительные фичи для лучшего распознавания
...
def prepare_improved_training_data():
    """Подготовка данных с улучшенными параметрами"""
    
    print("📊 Подготовка данных с улучшенными параметрами...")
    ...
    # Добавляем УЛУЧШЕННЫЕ TF-IDF фичи
    print("🔤 Добавление улучшенных TF-IDF фичей...")
    
    tfidf = TfidfVectorizer(
        max_features=1000,  # увеличили с 500
        ngram_range=(1, 4),  # увеличили с (1, 3)
        stop_words='english',
        min_df=1,  # уменьшили с 2
        max_df=0.95,  # увеличили с 0.9
        sublinear_tf=True,  # добавили
        norm='l2'  # добавили нормализацию
    )
...
def train_improved_model(X, y, tfidf):
    ...
    rf = RandomForestClassifier(
        n_estimators=500,  # увеличили
        max_depth=25,  # увеличили
        min_samples_split=2,  # уменьшили
        min_samples_leaf=1,
    ...
    gb = GradientBoostingClassifier(
        n_estimators=500,  # увеличили
        learning_rate=0.03,  # уменьшили для лучшей сходимости
    ...
    lr = LogisticRegression(
        ...
        C=0.01,  # уменьшили для лучшей регуляризации
        class_weight='balanced'
    )
    ...
    # Обучение
    print("Обучение улучшенного ансамбля...")
    ...
    print(f"Улучшенный Ensemble Accuracy: {accuracy:.3f}")
    
    # Детальный отчет
    print(f"\n📊 Детальный отчет по улучшенному ансамблю:")
...
def main():
    ...
    print("🚀 Обучение улучшенной модели Technology Evaluator...")
    print("=" * 60)
    
    # Подготовка данных
    X, y, tfidf = prepare_improved_training_data()
    
    # Обучение модели
    model, accuracy = train_improved_model(X, y, tfidf)
    
    # Сохранение модели
    print("\n💾 Сохранение улучшенной модели...")
    
    # Сохраняем модель
    joblib.dump(model, 'improved_technology_evaluator.joblib')
    
    # Сохраняем названия фичей
    with open('improved_technology_evaluator_features.json', 'w') as f:
        json.dump(list(X.columns), f)
    
    # Сохраняем TF-IDF
    joblib.dump(tfidf, 'improved_technology_evaluator_tfidf.joblib')
    
    print("✅ Улучшенная модель сохранена:")
    print("  - improved_technology_evaluator.joblib")
    print("  - improved_technology_evaluator_features.json")
    print("  - improved_technology_evaluator_tfidf.joblib")
    ...
    if prediction == 'commercial' and confidence > 0.3:
        print("✅ Улучшенная модель работает правильно!")
    else:
